...
`resizeOnWindowScroll` | true | Если `true`, то будет включено автоматические изменение высоты FancyBox в соответствии с высотой содержимого, `false` - центрирует FancyBox в области просмотра, когда ориентация устройства изменилась.
...
FancyBox генерирует события, аналогичные callback-функциям: `onStart`, `onCancel`, `onBeforeShow`, `onComplete`, `onCleanup`, `onClosed`, `onError`.
